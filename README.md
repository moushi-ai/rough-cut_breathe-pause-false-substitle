...
- **成片字幕可追溯导出** — 审核后从最终保留台词提取文本，再按纠错和断行规则生成字幕；不会回退到未剪的完整 ASR
...
如果还要成片字幕，必须在审核页导出后，从 `3_审核/review_log.json` 提取最终保留台词，再做纠错和断行；详细命令见 [`SKILL.md` 的“步骤 7B”](SKILL.md#步骤-7b从审核结果导出成片字幕)。这一步会输出 `4_字幕/retained_raw.txt`、`corrected.txt`、`uncertain.md` 与 `subtitles_formatted.md`。人名和专有名词不确定时默认保留 ASR 原文，不做未经确认的全篇替换。
