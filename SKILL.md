...
转录本身仍然一次完成；当时间轴 **达到或超过 120 秒**，不要等待全文语义分析结束才生成审核页。先规划审核批次：
...
- 输出 `mode: "single"`：视频短于两分钟，沿用原来的步骤 5.2–6（全文分析 → 一次性审核页）。
...
审核页和 FCPXML 导出共用 `scripts/lib/compute_keeps.js`。生成的静音/换气候选会先与逐字时间轴求交，只保留字与字之间的空隙；句头/句尾余量也会受字边界保护。保留片段内部只有静音达到 **0.4 秒**才会自动拆段。因此任何自动吸附或内部静音切割都不能落在一个正在发音的字中间。若 ASR 的字时间戳不准，默认宁可少切一点静音，也不切断发音。
...
3. 确认展示模式：默认是 `ASR 保真`；只有用户明确选择「正式成片字幕」或同义风格时，才启用正式展示层。不得从“需要字幕”或文件名推断该选择。
4. **只做纠错**，不断行；默认模式不改大小写、标点与空格。正式成片字幕模式仅按 `纠错prompt.md` 对已确认术语、千位数字及书名／引用做展示规范化，原始 ASR 保真稿必须保留且不被覆盖。
5. 输出到与输入同一流程的纠错目录：直接模式为 `$BASE_DIR/2_纠错/corrected.txt`；审核后成片字幕为 `$BASE_DIR/4_字幕/corrected.txt`（每行一句，与输入 1:1 对应）
6. 如有「不确定清单」，输出到同目录 `uncertain.md` 并在对话中列出给用户。人名、专有名词不确定时保留原 ASR，不得猜测后全篇替换。
