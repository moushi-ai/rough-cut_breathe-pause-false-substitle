 ...
 *   2) 句首：然后/那么/好的（两字）、哦/哎/呀/对/呢/那（单字，"那"需排除"那个/那么/那里"等）
 *   3) 任意位置：然后（用户偏好：宁可手动加回）
 *   4) 句尾：对、呢、哦
 *   规则 2/4 仅在删除后剩余 > 3-4 字时执行，避免句子被掏空
 *
 * 团队经验规则：自然语气词“啊”默认保留。独立/连续填充的“啊”由语义分析层
 * 按上下文标整句或词级候选，不能靠此处的位置硬规则删除。
 */
...
const EDGE_HEAD = new Set(['哦', '哎', '呀', '对', '呢']);
const EDGE_TAIL = new Set(['对', '呢', '哦']);
